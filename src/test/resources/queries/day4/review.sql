...
--How can we get information about theat employee (3rd min)
select * from EMPLOYEES where SALARY=(select max(salary) from (select distinct salary
                         from EMPLOYEES
                         order by SALARY asc)
where ROWNUM<4);

--Why we have 2 results in here. We already used distinct to make salaries unique?
--3rd min query will return one value of 2400.
--But we are looking for who has that salary
--2 colleagues has same salary that why it return 2 results